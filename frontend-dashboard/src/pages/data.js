...
const processAnalyticsData = (data) => {
    ...
    const dailyStats = Object.keys(groupedByDate).map(date => {
        ...
        for (let i = 1; i < dayData.length; i++) {
            ...
            if (timeDiffHours > 0 && timeDiffHours < 1) { // Prevent calculating over large gaps
    ...
    // If there's no data in the last 7 days, we can't show a summary.
...
export default function DataPage() {
  ...
  useEffect(() => {
    // Fetch data for analytics (last 14 days for week-over-week comparison potential)
    ...
    const unsubscribeAnalytics = onValue(analyticsQuery, (snapshot) => {
        ...
        setLoading(false);
    }, { onlyOnce: true }); // Use onlyOnce to prevent re-renders from live data

    // Fetch raw logs for the bottom table
